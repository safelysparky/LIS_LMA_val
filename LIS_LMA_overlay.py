...
for i, fname in enumerate(fname_list[0:1]):
    ...
    for ii, info in enumerate(big_flash_info):
        ...
        # ax.scatter(f_lon, f_lat, transform=ll_proj, c=f_t, zorder=11, marker='.', cmap='jet')
        
        # ax.scatter(E2.lon, E2.lat, transform=ll_proj, color='k', zorder=13, marker='.')
        ...
        # ## overlay lma sources and the fov
        # polygon_fov1=Polygon(fov1_lonlat)
        # x, y = polygon_fov1.exterior.xy
        # plt.plot(x, y, c="red")
        # plt.scatter(f_lon,f_lat,s=2,c='k')
        # plt.show()
        ...
        # fov = ltgLIS.get_fov(l.one_second, times=e.iloc[0].time) 
    ...
    #### print all stuff to a text file that can be used as input for data request
    

    flash_ID=69
    g1= l.get_groups(l.flashes[flash_ID].id, combine=True)
    
    # here I need to convert time stamps of each group into tod, to make the comparison with LMA easier
    group_t_stamps=g1['time']
    group_tod=group_t_stamps_to_tod(group_t_stamps)
              
    #find the invloved lma file:
    flash_DATE_STR=''.join(str(group_t_stamps.iloc[0])[:10].split('-'))
    involved_lma_files=find_involved_lma_file(NALMA_folder,LMA_NAME,flash_DATE_STR,group_tod[0],group_tod[-1])
    
    #load the lma, grab a chunk and do flash sorting, only read lma file again when it has not been read 
    for lma_fname in involved_lma_files:
        if lma_fname!=loaded_lma_fname:
            S_one_file=read_lma_format_data_as_nparray(lma_fname,NALMA_center[0],NALMA_center[1],0)
            loaded_lma_fname=lma_fname
            S_sorted,flash_info=flash_sorting(S_one_file,t_thres,d_thres,dd_method='2d')
    
    
    #find the invloved entln file:
    ENTLN_file=ENTLN_folder+flash_DATE_STR+'.npy'
    if os.path.exists(ENTLN_file) is True:
        EN=np.load(ENTLN_file)
    else:
    ...
    group_ID=688
    e1= l.get_events(l.groups[group_ID].id, combine=True)
    
    # By default, this function will get the corners of the footprint of
    # each event (pixel) you pass it.
    lla = ltgLIS.geolocate_events_in_frame(e1, l.one_second)
    ...
    ev_poly = ltgLIS.event_poly(e1, corners=lla, latlon=True, fill=True)
    ...
    ax.scatter(e1.lon, e1.lat, transform=ll_proj, color='black', zorder=11, marker='.')
